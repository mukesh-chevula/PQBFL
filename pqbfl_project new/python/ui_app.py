...
def main():
    ...
    if st.session_state.last_result is not None:
        ...
        if r.transaction_timings:
            ...
            tx_display["tx_hash"] = tx_display["tx_hash"].astype(str).str[:10] + "..."
            tx_display["round"] = tx_display["round"].astype(str)
            tx_display["client"] = tx_display["client"].apply(lambda value: "Server" if value == -1 else f"Client {value}")
            tx_display = tx_display[["tx_type", "round", "client", "duration_ms", "gas_used"]]
            tx_display.columns = ["Transaction Type", "Round", "Participant", "Duration (ms)", "Gas Used"]
            st.dataframe(tx_display, use_container_width=True, height=360)

            st.markdown("### Transaction type analysis")
            tx_stats = tx_df.groupby("tx_type").agg({"duration_ms": ["count", "mean", "min", "max", "std"], "gas_used": ["mean", "max"]}).round(2)
            tx_stats.columns = ["Count", "Avg Time (ms)", "Min Time (ms)", "Max Time (ms)", "Std Dev (ms)", "Avg Gas", "Max Gas"]
            st.dataframe(tx_stats, use_container_width=True)

            tx_chart_df = tx_df[["tx_type", "duration_ms"]].copy()
            tx_chart_df["transaction_number"] = range(1, len(tx_chart_df) + 1)
            tx_chart = (
                alt.Chart(tx_chart_df)
                .mark_circle(size=60)
                .encode(
        ...
        st.markdown("---")
        st.subheader("Off-chain operation timing")
        ...
        if r.operation_timings:
            op_df = pd.DataFrame(r.operation_timings)
            op_display = op_df.copy()
            op_display["round"] = op_display["round"].astype(str)
            op_display["client"] = op_display["client"].apply(lambda value: "Server" if value == -1 else f"Client {value}")
            op_display = op_display[["op_type", "round", "client", "duration_ms"]]
            op_display.columns = ["Operation", "Round", "Participant", "Duration (ms)"]
            st.dataframe(op_display, use_container_width=True, height=360)

            st.markdown("### Operation type analysis")
            op_stats = op_df.groupby("op_type").agg({"duration_ms": ["count", "mean", "min", "max", "std"]}).round(2)
            op_stats.columns = ["Count", "Avg Time (ms)", "Min Time (ms)", "Max Time (ms)", "Std Dev (ms)"]
            st.dataframe(op_stats, use_container_width=True)

            op_chart_df = op_df[["op_type", "duration_ms"]].copy()
            op_chart_df["operation_number"] = range(1, len(op_chart_df) + 1)
            op_chart = (
                alt.Chart(op_chart_df)
                .mark_circle(size=60)
                .encode(
