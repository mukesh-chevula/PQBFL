...
def _run_security_checks() -> list[dict]:
    """Run security checks for PQBFL Adaptive Ratcheting."""
    results = []
    ...
    try:
        ...
        key = os.urandom(32)
        nonce = os.urandom(12)
        ct, _ = aead_encrypt(key, b"secret", aad=b"aad", nonce=nonce)
        tampered = bytearray(ct)
        tampered[-1] ^= 0xFF
        try:
            aead_decrypt(key, bytes(tampered), aad=b"aad", nonce=nonce)
            results.append({"check": "AEAD tamper detection", "status": "❌ Fail", "detail": "Tampered ciphertext accepted"})
        except Exception:
            results.append({"check": "AEAD tamper detection", "status": "✅ Pass", "detail": "ChaCha20-Poly1305 correctly rejects tampered ciphertext"})
    ...
    try:
        ...
        results.append({"check": "Kyber KEM backend", "status": status, "detail": f"Backend: {backend}"})
    except Exception as e:
    ...
    try:
        ...
        kp = kyber_keygen()
        encap, _ = kyber_encap(kp.public_key)
        ss, _ = kyber_decap(encap.ciphertext, kp.secret_key)
        assert ss == encap.shared_secret
        results.append({"check": "Kyber KEM roundtrip", "status": "✅ Pass", "detail": "keygen → encap → decap: shared secrets match"})
    except Exception as e:
    ...
    try:
        ...
        ss1, _ = ecdh_shared_secret_x25519(a.private_key, b.public_key_bytes)
        ss2, _ = ecdh_shared_secret_x25519(b.private_key, a.public_key_bytes)
        assert ss1 == ss2
        results.append({"check": "ECDH key exchange", "status": "✅ Pass", "detail": "X25519 shared secrets match"})
    except Exception as e:
    ...
    try:
        ...
        kp = ed25519_keygen()
        sig, _ = ed25519_sign(kp.private_key, b"msg")
        assert ed25519_verify(kp.public_key_bytes, b"msg", sig) is True
        assert ed25519_verify(kp.public_key_bytes, b"wrong", sig) is False
        results.append({"check": "Ed25519 signatures", "status": "✅ Pass", "detail": "ed25519_verify checks signatures correctly"})
    except Exception as e:
        results.append({"check": "Ed25519 signatures", "status": "❌ Fail", "detail": str(e)})

    # 6. Safe model serialization
    try:
        ...
        results.append({"check": "Safe model serialization", "status": "✅ Pass", "detail": "npz format with allow_pickle=False"})
    except Exception as e:
        results.append({"check": "Safe model serialization", "status": "❌ Fail", "detail": str(e)})

    # 7. Adaptive ratcheting
    try:
        ...
        results.append({"check": "Adaptive ratcheting", "status": "✅ Pass", "detail": "ThreatMonitor + AdaptiveRatchetPolicy working correctly"})
    except Exception as e:
...
**This is the first implementation of threat-adaptive ratcheting in any post-quantum FL system.**
    ...
    with tab_security:
        ...
        mitigations = pd.DataFrame([
            {"#": 1, "Mitigation": "Constant-time Kyber KEM", "Module": "crypto/kyber.py", "Method": "pqcrypto (liboqs C backend)"},
            {"#": 2, "Mitigation": "Constant-time ECDH", "Module": "crypto/ecdh.py", "Method": "X25519 OpenSSL via cryptography lib"},
            {"#": 3, "Mitigation": "ChaCha20-Poly1305 authenticated encryption", "Module": "crypto/aead.py", "Method": "ChaCha20Poly1305 (cryptography lib)"},
            {"#": 4, "Mitigation": "Deterministic nonces", "Module": "crypto/aead.py", "Method": "hash32(seed)[:12]"},
            {"#": 5, "Mitigation": "Constant-time signature verification", "Module": "crypto/eddsa.py", "Method": "ed25519_verify"},
            {"#": 6, "Mitigation": "Safe model serialization", "Module": "fl/model.py", "Method": "numpy npz (allow_pickle=False)"},
            {"#": 7, "Mitigation": "Key validation", "Module": "crypto/aead.py", "Method": "Strict 32-byte check, no branching"},
            {"#": 8, "Mitigation": "Adaptive ratcheting", "Module": "adaptive/", "Method": "ThreatMonitor + AdaptiveRatchetPolicy"},
        ])
